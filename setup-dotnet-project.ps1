...
if (-not (Check-DotnetVersion)) {
    ...
    if ($response.ToLower() -ne "y") {
        Write-Host "Script execution stopped. .NET 9: https://dotnet.microsoft.com/en-us/download/dotnet/9.0"
        exit 1
    }
}
...
$efToolExists = (dotnet tool list -g | Select-String "dotnet-ef").Count -gt 0
if ($efToolExists) {
    Write-Host "dotnet-ef is already installed. Updating..."
